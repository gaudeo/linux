				  ...
				  struct smb347_charger_platform_data *pdata)
{
	...
	of_property_read_u32(np, "charge-term-current-microamp",
			     &pdata->termination_current);
	of_property_read_u32(np, "precharge-current-microamp",
			     &pdata->pre_charge_current);
	of_property_read_u32(np, "constant-charge-current-max-microamp",
			     &pdata->max_charge_current);
	of_property_read_u32(np, "constant-charge-voltage-max-microvolt",
			     &pdata->max_charge_voltage);
	of_property_read_u32(np, "summit,fast-volt-threshold-microvolt",
			     &pdata->pre_to_fast_voltage);
	of_property_read_u32(np,
		"summit,mains-constant-charge-current-max-microamp",
		&pdata->mains_current_limit);
	of_property_read_u32(np,
		"summit,usb-constant-charge-current-max-microamp",
		&pdata->usb_hc_current_limit);

	/* For thermometer monitoring */
	of_property_read_u32(np, "summit,chip-temp-threshold-celsius",
			     &pdata->chip_temp_threshold);
	if (of_property_read_u32(np, "summit,soft-cold-temp-limit-celsius",
				 &pdata->soft_cold_temp_limit))
		pdata->soft_cold_temp_limit = SMB347_TEMP_USE_DEFAULT;
	if (of_property_read_u32(np, "summit,soft-hot-temp-limit-celsius",
				 &pdata->soft_hot_temp_limit))
		pdata->soft_hot_temp_limit = SMB347_TEMP_USE_DEFAULT;
	if (of_property_read_u32(np, "summit,hard-cold-temp-limit-celsius",
				 &pdata->hard_cold_temp_limit))
		pdata->hard_cold_temp_limit = SMB347_TEMP_USE_DEFAULT;
	if (of_property_read_u32(np, "summit,hard-hot-temp-limit-celsius",
